...
// POST /api/timeline/reorder — persist a new drag-and-drop order
...
export async function POST(request: NextRequest) {
  ...
  const results = await Promise.all(
    orderedIds.map((id, index) =>
      supabase.from("timeline_events").update({ display_order: index + 1 }).eq("id", Number(id))
    )
  )

  const failed = results.find((r) => r.error)
  if (failed?.error) {
    console.error("Error reordering timeline_events:", failed.error)
